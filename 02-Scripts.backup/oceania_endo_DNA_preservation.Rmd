---
title: "Pacific calculus endogenous content preservation"
author: "Irina Velsko"
...
calculus_pacific <- fread("./05-Documentation.backup/sink_predictions_species.txt")
...
ggsave("./06-publication/main_figures/Figure_XX1/endo_dna_preservation.svg", plot = wrap_plot, device = "svg",
...
comb_pacific %>%
  ...
  arrange(Type) %>%
  # mutate(Percent = Percent + 1) %>%
  ggplot(., aes(x = Site, y = Percent, fill = Type)) +
    geom_boxplot(color = "grey30") +
    geom_jitter() +
    scale_fill_manual(values = c("#7d3560","#148f77")) +
    theme_minimal() +
...
# ggsave("./06-publication/main_figures/Figure_XX1/endo_dna_preservation.svg", plot = combined_plot, device = "svg",
#        scale = 1, width = 10, height = 5, units = c("in"), dpi = 300)
