---
title: "Pacific calculus HUMAnN3 species contributions to PC1 paths"
author: "Irina Velsko"
...
humann3_pathway_biplot_list <- fread("./04-Analysis/humann3/humann3_pathway_biplot_list.tsv") 
...
humann3_path_pcbi_bar_df %>%
  ...
  mutate(Genus = fct_relevel(Genus, "unclassified","Other"), # ,"Kosakonia","Lautropia","Neisseria","Ottowia","Pseudomonas","Pseudopropionibacterium")
...
humann3_path_pcbi_bar_df %>%
  ...
  filter(Percent >= 5 | (Percent <= 5 & Genus == "Other")) %>% # filter out the genera with % < 5, but keep Other < 5
  mutate(Genus = fct_relevel(Genus, "unclassified","Other"), # ,"Methanobrevibacter","Pseudomonas","Streptococcus"
         Path = fct_relevel(Path, humann3_pathway_biplot_list %>%
                              filter(Direction == "PC1-") %>%
                              pull(Path))) %>%
  ggplot(., aes(x=Path, y=Percent, fill = Genus)) +
...
humann3_path_pcbi_bar_df %>%
  ...
  filter(Percent >= 5 | (Percent <= 5 & Genus == "Other")) %>% # filter out the genera with % < 5, but keep Other < 5
  mutate(Genus = fct_relevel(Genus, "unclassified","Other"), # ,"Aggregatibacter","Bacteroidetes_unclassified","Clostridiales_Family_XIII_Incertae_Sedis_unclassified","Delftia","Desulfobulbus","Desulfomicrobium","Kosakonia","Haemophilus","Mogibacterium","Ottowia","Pseudomonas","Pseudoramibacter","Porphyromonas","Streptococcus","Tannerella"
  Path = fct_relevel(Path, humann3_pathway_biplot_list %>%
                       filter(Direction == "PC2+") %>%
                       pull(Path))) %>%
  ggplot(., aes(x=Path, y=Percent, fill = Genus)) +
...
humann3_path_pcbi_bar_df %>%
  filter(Direction == "PC2-") %>% # removing Other plots all species/unassigned - no need to filter the pathways
  filter(Percent >= 5 | (Percent <= 5 & Genus == "Other")) %>% # filter out the genera with % < 5, but keep Other < 5
  mutate(Genus = fct_relevel(Genus, "unclassified","Other"), # ,"Delftia","Kosakonia","Methanobrevibacter","Neisseria","Pseudomonas","Pseudopropionibacterium","Streptococcus","Streptomyces"
         Path = fct_relevel(Path, humann3_pathway_biplot_list %>%
                              filter(Direction == "PC2-") %>%
                              pull(Path))) %>%
  ggplot(., aes(x=Path, y=Percent, fill = Genus)) +
