...
## ** Find this file!! ** ##
stats <- read.delim("~/Dropbox (MPI SHH)/2-Calculus_Oceania/4-Results/phylogenies/pseudopropionibacterium_propionicum/pseudo_pro_eager_stats.txt")
...
# Add ECO002 back in (it's bad, but I need an outgroup to root with)
eco <- stats %>%
  filter(Sample.Name == "ECO002.B0102") %>%
  select(Sample.Name)
samples <- full_join(samples, eco)

# Remove EMN if necessary
samples <- samples %>%
  filter(!Sample.Name == "EMN001.A0102")
```

```{r}
# read in metadata for adding later
pacific_metadata <- fread("05-Documentation.backup/oceania_metadata_full.tsv") %>%
  select(Library_ID, Lab, Island, Type, Age_mean) %>%
...
```{r pp}
## Set variables
in_file <- "./04-Analysis/phylogenies/pseudopropionibacterium_propionicum/multivcfanalyzer/snpAlignment.fasta"
in_file_hom <- "./04-Analysis/phylogenies/pseudopropionibacterium_propionicum/multivcfanalyzer_hom/snpAlignment.fasta"
min_snps <- 1000
model <- "JC69"
n_boots <- 100
exclude <- "list"

snp_stats <- fread("./04-Analysis/phylogenies/pseudopropionibacterium_propionicum/multivcfanalyzer/snpStatistics.tsv", skip = 4, sep = "\t", header = T)
...
## Report number of pairwise overlapping bases (i.e. how many nucleotides shared between two samples, of these which are same and different bases)
...
## Root tree
tre_pp <- root(tre_pp, outgroup = "ECO002.B0102")

## Add bootstraps to tree
tre_pp$node.label <- boots
...
tre_pp$edge.length[tre_pp$edge.length < 0] <- 0

# Now laderize the rooted tree. To make a more aesthetically pleasing tree it is sometimes useful to ladderize the tree, 
tre1 <- ladderize(tre_pp, right=TRUE)  # TRUE and FALSE change direction

# and check if tree loaded correctly
plot(tre_pp)

p_pp <- ggtree(tre_pp, ladderize = F) # For rectangular tree
p1_pp <- p_pp + geom_tiplab(size = 2)
p1_pp


p2_pp <- ggtree(tre_pp, ladderize = T) %<+% pacific_metadata
p2_pp_tree <- p2_pp + geom_tippoint(aes(fill = Age_mean, shape = Study, size = SNPs)) +
  geom_tiplab(aes(color = Island), size = 2, offset = 0.005) + # , as_ylab=TRUE
...
tip_order <- get_taxa_name(p2_pp_tree)
...
tree_pp <- ggtree(tre_pp, branch.length="none") +
  geom_tiplab(size = 2) +
