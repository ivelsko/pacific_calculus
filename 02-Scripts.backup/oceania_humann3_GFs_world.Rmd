...
humann3_gf_l1_noblanks_filt.clr.mat.df <- humann3_gf_l1_noblanks_filt.clr.mat %>%
  as.data.frame.matrix() %>%
  rownames_to_column("KOs") %>%
  pivot_longer(!KOs, names_to = "Samples", values_to = "cpm") %>%
  pivot_wider(names_from = "KOs", values_from = "cpm") %>%
  column_to_rownames("Samples") 

res.km <- factoextra::eclust(humann3_gf_l1_noblanks_filt.clr.mat.df, "kmeans")

gap_line_samples <- factoextra::fviz_gap_stat(res.km$gap_stat)
gap_line_samples


factoextra::fviz_cluster(res.km, humann3_gf_l1_noblanks_filt.clr.mat.df, geom = "point")

checksamples <- res.km$clust_plot$data %>%
  as.data.frame() %>%
  rename(Library_ID = name) %>%
  left_join(., metadata %>%
              full_join(., pacific_metadata %>%
                          mutate(Sample_type = "calculus",
                                 Sample_group = "ancient_calculus")), by = "Library_ID") 

checksamples %>%
  ggplot(., aes(x = x, y = y, fill = cluster, shape = Sample_type)) +
         geom_point(size =  2, stroke = 0.1) + #
         scale_shape_manual(values = c(21,25)) +
         scale_fill_manual(values = c("#098BD9", "#7472af", "#cc79a7", "#f6865e", "#1ec1a7", "#86c73a")) +
         theme_minimal() +
         # ylim(0,100) +
...
gap_cluster_samples <- checksamples %>%
  ggplot(., aes(x = x, y = y, color = cluster, shape = cluster)) +
         geom_point(size = 1.5) + #
         scale_color_manual(values = c("#098BD9", "#7472af", "#cc79a7", "#f6865e", "#1ec1a7", "#86c73a")) +
         scale_shape_manual(values = c(3,19,17,15,7,8)) +
         theme_minimal() +
         # ylim(0,100) +
         # scale_y_continuous(breaks=scales::pretty_breaks(n=10)) +
         ylab("Dim2 (12.8%)") +
...
# ggsave("./06-publication/supplemental_figures/Sup_fig_XX17/gap_cluster_samples.pdf", plot = gap_samples, device = "pdf",
#        scale = 1, width = 8.5, height = 5, units = c("in"), dpi = 300)


gap_samples <- gap_line_samples / gap_cluster_samples +
  plot_layout(heights = c(1,1.5))
gap_samples

ggsave("./06-publication/supplemental_figures/Sup_fig_XX17/gap_cluster_samples.pdf", plot = gap_samples, device = "pdf",
       scale = 1, width = 4.5, height = 5, units = c("in"), dpi = 300)

```

```{r cluster_KOs}

humann3_gf_l1_noblanks_filt.KO.clr.mat.df <- humann3_gf_l1_noblanks_filt.clr.mat %>%
  as.data.frame.matrix()

res_KO.km <- factoextra::eclust(humann3_gf_l1_noblanks_filt.KO.clr.mat.df, "kmeans")

gap_line_KOs <- factoextra::fviz_gap_stat(res_KO.km$gap_stat)
gap_line_KOs

factoextra::fviz_cluster(res_KO.km, humann3_gf_l1_noblanks_filt.KO.clr.mat.df, geom = "point")

checksamples_KO <- res_KO.km$clust_plot$data %>%
  as.data.frame()  

gap_cluster_KOs <- checksamples_KO %>%
  ggplot(., aes(x = x, y = y, color = cluster)) +
         geom_point(size =  1, stroke = 0.1) + #
         scale_color_manual(values = c("#098BD9", "#7472af", "#cc79a7", "#f6865e", "#1ec1a7", "#86c73a", "#616161", "#e1f2fe", "#f5c710ff", "#7D3560")) +
         scale_shape_manual(values = c(3,19,17,15,7,8)) +
         theme_minimal() +
         # ylim(0,100) +
         # scale_y_continuous(breaks=scales::pretty_breaks(n=10)) +
...
gap_KOs <- gap_line_KOs / gap_cluster_KOs +
  plot_layout(heights = c(1,1.5))
gap_KOs

ggsave("./06-publication/supplemental_figures/Sup_fig_XX17/gap_cluster_KOs.pdf", plot = gap_KOs, device = "pdf",
       scale = 1, width = 4.5, height = 5, units = c("in"), dpi = 300)
