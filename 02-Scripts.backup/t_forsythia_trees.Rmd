---
title: "Tannerella forsythia Pacific trees"
...
stats %>%
  filter(Mean.cov <= 2) %>%
  select(Sample.Name) %>%
  fwrite(., "./04-Analysis/phylogenies/gubbins/exclude_tf_less_5x_cov_samples.tsv", quote = F, sep = "\t", col.names = F)
...
```{r tf}
## Set variables
in_file <- "./04-Analysis/phylogenies/tannerella_forsythia/multivcfanalyzer/snpAlignment.fasta"
in_file_hom <- "./04-Analysis/phylogenies/tannerella_forsythia/multivcfanalyzer_hom/snpAlignment.fasta"
min_snps <- 1000
model <- "JC69"
n_boots <- 100
exclude <- "list"
...
in_file_full_hom <- "./04-Analysis/phylogenies/tannerella_forsythia/multivcfanalyzer_hom/fullAlignment.fasta"
...
# write.FASTA(subset_full_fasta, "./05-Documentation.backup/subset_tf_fullAlignment.fasta")
...
tree_tf <- ggtree(tre_tf, branch.length="none") +
  geom_tiplab(size = 2) +
  #ggtitle("Tannerella forsythia, >4X coverage") +
  ggplot2::xlim(0, 60)
...
ggsave("./06-publication/main_figures/Figure_XX5/tf_tree.pdf", plot = tf_tree_plots, device = "pdf",
